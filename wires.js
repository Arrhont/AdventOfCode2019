...
class Wire {
    ...
    constructor(descriptorArray = []) {
        ...
        for (let i = 0; i < descriptorArray.length; i++) {
            this.setByDescriptor(descriptorArray[i]);
        }
    }
    ...
    setByDescriptor(descriptor) {
        let opcode = descriptor[0];
        let length = '';
        for (let i = 1; i < descriptor.length; i++) {
            length = length + descriptor[i];     
        }
        length = +length;
        ...
        switch (opcode) {
            ...
            case 'R':
                ...
                break;
            default:
                throw new Error(`Unknown descriptor directive ${opcode}`);
            }
    }

    static getIntersections(wire1, wire2) {
        let intersections = wire1.coordinates.reduce((acc, coords, index) => {
            for (let i = 0; i < wire2.coordinates.length; i++) {
                if (coords[0] === wire2.coordinates[i][0] && coords[1] === wire2.coordinates[i][1]) {
                    acc.push([...coords, index, i]);
                }
            }
            return acc;
        }, []);
...
function intersectionWiresLength(coords) {
    return coords[2] + coords[3];
}


let wire1 = new Wire(wire1DirectionsArray);
let wire2 = new Wire(wire2DirectionsArray);

const intersections = Wire.getIntersections(wire1, wire2);

let closestIntersection = intersections.reduce((acc, coords) => {
    if (acc > manhattanDistance(coords)) {
        acc = manhattanDistance(coords);
    }
    return acc;
}, manhattanDistance(intersections[0]));

let closestLength = intersections.reduce((acc, coords) => {
    if (acc > intersectionWiresLength(coords)) {
        acc = intersectionWiresLength(coords);
    }
    return acc;
}, intersectionWiresLength(intersections[0]));

console.log(intersections);
console.log(closestIntersection);
console.log(closestLength);